// recreating svg size / attribute ecentricities
// when logotype png was converted to svg
var w = 1132,
  h = 372

var svg = d3
  .select('#viz-google-dots .svg-holder')
  .append('svg')
  .attr('class', 'logo')
  .attr('width', w)
  .attr('height', h)
...
d3.json('/assets/data/google.json', function(data) {
  ...
  svg
    ...
    .attr('d', function(d) {
      return path(d.letter_coords)
    })

  // initial transition trigger
  timeout = setTimeout(function() {
    d3.select('input[value="dots"]').property('checked', true).each(change)
  }, 2000)
})

// handle input changing
d3.selectAll('#viz-google-dots input').on('change', change)
...
// compute circle coordinates for shape tween
